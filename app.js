...
if (parseInt(BLOG_CONFIG.postsPerPage) > 19) {
  console.warn("SoloHive: postsPerPage is capped at 19 due to Hive API limit.");
}

// Normalize siteUrl — remove trailing slash to prevent double slashes in URLs
...
function initPage(isPost) {
  // Apply configured language to html element for screen readers
  if (BLOG_CONFIG.htmlLang) {
    document.documentElement.lang = BLOG_CONFIG.htmlLang;
  }
          ...
          <div class="share-section">
            ...
            <div class="share-buttons">
              <a class="share-btn share-twitter"
                 href="https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=${encodeURIComponent(siteUrl + '/post.html?author=' + post.author + '&permlink=' + post.permlink)}"
              ...
              <a class="share-btn share-facebook"
                 href="https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(siteUrl + '/post.html?author=' + post.author + '&permlink=' + post.permlink)}"
                 target="_blank" rel="noopener" title="Share on Facebook">
                <svg viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                Facebook
              </a>
              <a class="share-btn share-linkedin"
                 href="https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(siteUrl + '/post.html?author=' + post.author + '&permlink=' + post.permlink)}"
              ...
              <button class="share-btn share-copy"
                onclick="copyPostLink('${siteUrl + "/post.html?author=" + post.author + "&permlink=" + post.permlink}')"
                title="Copy link">
...
// ─────────────────────────────────────────────────────────────────────────────
//  AUTO-INIT
//  Scripts load at the bottom of <body> so the DOM is already ready.
